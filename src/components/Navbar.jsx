...
const Navbar = () => {
  ...
  return (
    <React.Fragment>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 1000, background: isScrolled ? 'rgba(255, 255, 255, 0.95)' : 'rgba(255, 255, 255, 0.98)', backdropFilter: isScrolled ? 'blur(20px)' : 'blur(10px)', boxShadow: isScrolled ? '0 8px 32px rgba(0, 0, 0, 0.12)' : '0 2px 8px rgba(0, 0, 0, 0.05)', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', padding: isScrolled ? '0.75rem 0' : '1rem 0', borderBottom: '1px solid rgba(74, 144, 226, 0.15)' }}>
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div className="desktop-menu" style={{ display: 'none', gap: '0.5rem', alignItems: 'center' }}>
            ...
            <div className="dropdown-wrapper" onMouseEnter={() => setShowSegurosDropdown(true)} onMouseLeave={closeAllMenus} style={{ position: 'relative' }}>
              <button className="nav-link" style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.25rem', color: '#1F2937', fontWeight: '600', fontSize: '0.9375rem', padding: '0.75rem 1.25rem' }}>Nuestros seguros<ChevronDown size={16} /></button>

              {showSegurosDropdown && (
                <div className="dropdown-menu" style={{ position: 'absolute', top: '100%', left: '0', marginTop: '0.5rem', background: 'white', borderRadius: '0.75rem', boxShadow: '0 10px 40px rgba(0, 0, 0, 0.15)', padding: '0.5rem', minWidth: '280px', border: '1px solid rgba(74, 144, 226, 0.2)', zIndex: 1001 }}>
                  
                  <div className="dropdown-item-wrapper" onMouseEnter={() => setShowPersonasSubmenu(true)} onMouseLeave={() => setShowPersonasSubmenu(false)} style={{ position: 'relative' }}>
                    <a href="/seguros-para-personas" className="dropdown-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 1rem', color: '#1F2937', textDecoration: 'none', borderRadius: '0.5rem', fontSize: '0.9375rem', fontWeight: '500' }}>Seguros para Personas<ChevronDown size={14} style={{ transform: 'rotate(-90deg)', opacity: 0.5 }} /></a>
                    {showPersonasSubmenu && (
                      <>
                        <div style={{ position: 'absolute', left: '100%', top: 0, bottom: 0, width: '10px', zIndex: 1000 }} />
                        <div className="submenu" style={{ position: 'absolute', left: '100%', top: '0', marginLeft: '0.5rem', background: 'white', borderRadius: '0.75rem', boxShadow: '0 10px 40px rgba(0, 0, 0, 0.15)', padding: '0.5rem', minWidth: '300px', maxHeight: '500px', overflowY: 'auto', border: '1px solid rgba(74, 144, 226, 0.2)' }}>
                  ...
                  <div className="dropdown-item-wrapper" onMouseEnter={() => setShowEmpresasSubmenu(true)} onMouseLeave={() => setShowEmpresasSubmenu(false)} style={{ position: 'relative' }}>
                    <a href="/seguros-para-empresas" className="dropdown-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 1rem', color: '#1F2937', textDecoration: 'none', borderRadius: '0.5rem', fontSize: '0.9375rem', fontWeight: '500' }}>Seguros para Empresas<ChevronDown size={14} style={{ transform: 'rotate(-90deg)', opacity: 0.5 }} /></a>
                    {showEmpresasSubmenu && (
                      <>
                        <div style={{ position: 'absolute', left: '100%', top: 0, bottom: 0, width: '10px', zIndex: 1000 }} />
                        <div className="submenu" style={{ position: 'absolute', left: '100%', top: '0', marginLeft: '0.5rem', background: 'white', borderRadius: '0.75rem', boxShadow: '0 10px 40px rgba(0, 0, 0, 0.15)', padding: '0.5rem', minWidth: '320px', maxHeight: '500px', overflowY: 'auto', border: '1px solid rgba(74, 144, 226, 0.2)' }}>
      ...
      <style>{`
        .nav-link { color: #1F2937; text-decoration: none; fontWeight: 600; fontSize: 0.9375rem; padding: 0.75rem 1.25rem; borderRadius: 0.5rem; transition: all 0.3s ease; }
        .nav-link:hover { color: #0066CC; background: rgba(74, 144, 226, 0.1); }
        .dropdown-item:hover { background: rgba(74, 144, 226, 0.1); color: #0066CC; }
        .submenu-item { display: block; padding: 0.75rem 1rem; color: #1F2937; textDecoration: none; borderRadius: 0.5rem; fontSize: 0.875rem; fontWeight: 500; transition: all 0.2s ease; }
        .submenu-item:hover { background: rgba(74, 144, 226, 0.1); color: #0066CC; paddingLeft: 1.25rem; }
        .mobile-item { display: block; color: #1F2937; textDecoration: none; fontWeight: 600; fontSize: 1.0625rem; padding: 0.875rem 1rem; borderRadius: 0.75rem; borderBottom: 1px solid rgba(74, 144, 226, 0.1); }
